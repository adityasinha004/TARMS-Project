...
routes.get("/home/RequestForm", Auth, async (req, res) => {
    ...
    if (!result.uid || result.uid === "") {
        res.send("Error: User ID is missing. Please login again and try booking.");
        return;
    }

    if (!result.id || result.id === "") {
        res.send("Error: Room ID is missing. Please go back to home page and try again.");
        return;
    }
...
routes.get("/faculty/prediction", Auth, (req, res) => {
    exec("./venv/bin/python src/predict_usage.py", (error, stdout, stderr) => {
        ...
        // Get user details for the view
        let user = detail2(req.query.uid);
        user.then(data => {
            res.render("prediction", {
                uid: req.query.uid,
                name: data.fac_name,
                backUrl: `/faculty?uid=${req.query.uid}`
            });
        }).catch(err => {
            console.log(err);
            res.send("Some error occured");
    ...
    exec("./venv/bin/python src/predict_usage.py", (error, stdout, stderr) => {
        ...
        user.then(data => {
            res.render("prediction", {
                uid: req.query.uid,
                name: data.Name,
                backUrl: `/home?uid=${req.query.uid}`
            });
        }).catch(err => {
            console.log(err);
            res.send("Some error occured");
